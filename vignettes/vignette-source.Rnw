...
\usepackage{rotating}
\SweaveOpts{out.width = "\\maxwidth", size = "footnotesize", comment = NA} 
...
\parindent=0pt % no indent throught the document
...
This vignette documents workflow and results from our paper^{\cite{skrbinsek2012}}.

\section*{Installing the package \texttt{resamplediversity}}
Our package has one dependency, a package called \texttt{adegenet}. On Windows, you can install 
...
\texttt{file.choose()} you can specify a (full) path to the .zip file. 
If you are using R GUI, you can install by clicking \texttt{Packages > Install package(s) from local zip file}
and navigate to the downloaded file (make sure you have \texttt{adegenet} installed). 
Ultimately, you can build from the \textbf{source} tarball on any operating system platform. Source is
available on request from package author (\texttt{tomaz.skrbinsek@gmail.com}) or 
maintainer (\texttt{roman.lustrik@gmail.com}).

\vspace{0.4cm}

\noindent Contact us if you have a problem installing the package.

% how to install if you have build tool chain ready
...
Genetic diversity study of this population included samples of 50 individuals. We need to subset the locus panel using generic names of loci:
<<look_genericLoci, echo = TRUE, eval = TRUE, tidy = FALSE>>=
...
We can see that allelic richness is 21\% higher in Kluane than in Dinaric Mountains, and heterozgosity 4\%.

We can now batch-run the corrections for the entire set of North American populations studied 
by Paetkau et al. using the same locus set:
...
print(xtable(na.pops, caption = "North American populations of brown bears studied by Paetkau et al.", label = "table:na.pops"),
...
<<print_na, echo = TRUE, eval = FALSE, tidy = FALSE>>=
#these are resampled value for the reference population, hence prefix "ref".
names(adjusted_na) <- paste("ref", names(adjusted_na),
                            sep = "") 
pops.adjusted_na <- cbind(na.pops, adjusted_na)
@
...
%Bine: prišlo je do večkratnega cbindanja - veliko podvojenih stolpcev. 
%Sem spremenil kodo in naredil nov df pops.adjusted_na

%\newpage
%<<echo = FALSE, tidy = FALSE, results = "asis">>=
%print(xtable(pops.adjusted_na, caption = "Results for correction of the entire set of North American populations studies (Paetkau et al.).",
...
Ar.na <- with(pops.adjusted_na, 
              calcDivRat(ref = refA, SEref = refSEA, obs = A,
                         SEobs = SEA, type = "A"))
Her.na <-  with(pops.adjusted_na,
                calcDivRat(ref = refHe, SEref = refSEHe,
                           obs = He, SEobs = SEHe, type = "He"))
pops.adjusted_na.out <- cbind(pops.adjusted_na, Ar.na, Her.na)
pops.adjusted_na.out[, c("Population","Ar","SEAr","Her","SEHer")]
@
...
adjusted_cant <- runall(N = cant.pops$N,
                        ...
                        loci = loci_cant,
                        nboots = 100)
names(adjusted_cant) <- paste("ref", names(adjusted_cant),
                              sep = "")
pops.adjusted_cant <- cbind(cant.pops, adjusted_cant)

Ar.cant <- with(pops.adjusted_cant,
                calcDivRat(ref = refA, SEref = refSEA, obs = A,
                        SEobs = SEA, type = "A"))
Her.cant <- with(pops.adjusted_cant,
                 calcDivRat(ref = refHe, SEref = refSEHe,
                            obs = He, SEobs = SEHe, type = "He"))
pops.adjusted_cant.out <- cbind(pops.adjusted_cant, Ar.cant,
                                Her.cant)

pops.comparison <- rbind(
          pops.adjusted_na.out,
          pops.adjusted_cant.out)

pops.comparison[, c("Population","Ar","SEAr","Her","SEHer")]
@

%Bine: ta del spodaj (zakomentiran) ni ok. Zadeva v štartu ni delala, 
%ker se je adjusted_na večkrat cbindal. 
%treba je na novo naredil adjusted_na, pa  bo ok. Isti hec bo
%adjusted_cant. Naredil sem nova objekta pops.adjusted_na in 
%pops.adjusted_cant, ki bi morala to rešit.
%
%adjusted_na_out <- pops.adjusted_na[, c("Population", "Ar", "SEAr",
%                                  "Her", "SEHer")]
...
print(xtable(pops.comparison[, c("Population","Ar","SEAr","Her","SEHer")], caption = "reference-population calibrated diversity ratios for North American and Cantabrian (Spain) populations.",
...
Only resampling reference population corrections are done. You can calculate Ar and Her on your own as an exercise (see the example in previous section).
...
loci_nor <- c("L02", "L03", "L04", "L06",
              "L07", "L08", "L09", "L10")    
Nsamples_nor <- c(28, 50, 119, 148, 40, 41, 55, 45,
                  58, 30, 36, 57, 35, 34)  
adjusted_nor <- runall(N = Nsamples_nor,
                      genotypes = dinaric.genotypes,
                      loci = loci_nor,
                      nboots = 1000)
@
